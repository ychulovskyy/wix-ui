...
describe.only('googleRequestHandler', async () => {
    beforeEach(() => {
        ...
        googleZeroResultsMock = GoogleMapsMock(
            {
                getPlacePredictions: (request, callback) => {
                    callback(zeroResults, dummyGoogleMapsMock.maps.GeocoderStatus.ZERO_RESULTS);
                }
            },
            {
                geocode: (request, callback) => {
    ...
    it('should return an empty array when status=ZERO_RESULTS for autocomplete', () => {
        initRequestHandler(googleZeroResultsMock);
        ...
        expect(postMessageMock).toHaveBeenCalledWith(
            {
                results: zeroResults,
                status: dummyGoogleMapsMock.maps.GeocoderStatus.ZERO_RESULTS,
                requestId: mockRequestId
            }, '*'
        );
    });

    it('should return an empty array when status=ZERO_RESULTS for geocode', () => {
        initRequestHandler(googleZeroResultsMock);
    ...
    it('should return an empty array when status=ZERO_RESULTS for placeDetails', () => {
        initRequestHandler(googleZeroResultsMock);

...
describe.only('GoogleMapsIframeClient', () => {
    ...
    it('should resolve 2 requests', async () => {
        ...
        iframeManagerPrototype.getIframe.mockImplementation(() => ({
                postMessage: requestObj => {
                    ...
                    setTimeout(() => (<any>global).postMessage({
                        requestId,
                        results: [request],
                        status: 'OK'
                    }, targetOrigin), timeout);
                }
            })
        );
    ...
    it('should reject a request when it is cancelled', async () => {
        const postMessageMock = jest.fn();
        const address = '1 Broadway NYC';
        ...
        client = new GoogleMapsIframeClient();
        const autocompleteRequest = client.autocomplete(mockApiKey, mockLang, address);
        const geocodeRequest = client.geocode(mockApiKey, mockLang, address);
        const placeDetailsRequest = client.geocode(mockApiKey, mockLang, address);

        client.cancel(autocompleteRequest);
        client.cancel(geocodeRequest);
        client.cancel(placeDetailsRequest);

        await expect(autocompleteRequest).rejects.toEqual({
            isCancelled: true,
            error: 'Request cancelled'
        });
        await expect(geocodeRequest).rejects.toEqual({
            isCancelled: true,
            error: 'Request cancelled'
        });
        await expect(placeDetailsRequest).rejects.toEqual({
            isCancelled: true,
            error: 'Request cancelled'
        });
